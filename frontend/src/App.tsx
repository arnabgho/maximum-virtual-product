...
export default function App() {
  const { user, loading: authLoading, signInWithGitHub, signOut, initialize: initAuth } = useAuthStore();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg-deep)]">
        ...
        <div className="flex flex-col items-center gap-4 relative z-10">
          <svg className="w-8 h-8 animate-spin text-[var(--accent-cyan)]" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
          </svg>
          <span className="font-mono-hud text-xs text-[var(--text-muted)] uppercase tracking-widest">Initializing...</span>
        </div>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg-deep)] relative">
        <div className="grid-bg" />
        <div className="ambient-glow" />
        ...
        >
          ...
          <div className="text-center">
            <h1 className="font-mono-hud text-2xl font-bold tracking-tight text-[var(--text-primary)] mb-2">
              <span className="text-glow">MAXIMUM VIRTUAL BLUEPRINT</span>
            </h1>
            <p className="font-mono-hud text-xs text-[var(--accent-cyan)] uppercase tracking-[0.3em]">
              AI Research Core
            </p>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-3 px-6 py-3.5 hud-btn-primary rounded-lg font-mono-hud text-sm font-bold uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-transform"
          >
          ...
          <p className="font-mono-hud text-[10px] text-[var(--text-muted)] uppercase tracking-wider">
            Authenticate to access your projects
          </p>
        </motion.div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg-deep)] relative pb-7">
        ...
        <div className="fixed top-0 left-0 right-0 h-10 bg-[var(--bg-surface)]/80 backdrop-blur-sm border-b border-[var(--border-dim)] flex items-center px-5 z-20">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            {user && (
              <div className="flex items-center gap-2 ml-2 pl-3 border-l border-[var(--border-dim)]">
                {user.user_metadata?.avatar_url && (
                  <img
                    src={user.user_metadata.avatar_url}
                    alt=""
                    className="w-5 h-5 rounded-full ring-1 ring-[var(--border-dim)]"
                  />
                )}
                <button
                  onClick={signOut}
                  className="font-mono-hud text-[10px] text-[var(--text-muted)] hover:text-[var(--accent-red)] uppercase tracking-wider transition-colors"
                >
                  Sign Out
                </button>
              </div>
